...
### 算法原理
> :pencil2:LZ77压缩算法采用字典的方式进行压缩，是一个简单但十分高效的数据压缩算法。其方式就是把数据中一些可以组织成短语(最长字符)的字符加入字典，然后再有相同字符出现采用标记来代替字典中的短语，如此通过标记代替多数重复出现的方式以进行压缩。

---
### 关键词
{{< admonition note "向前缓冲区">}}
 :black_nib:每次读取数据的时候，先把一部分数据预载入前向缓冲区。为移入滑动窗口做准备
{{< /admonition >}}
</br>
{{< admonition note "滑动窗口">}}
 :black_nib:一旦数据通过缓冲区，那么它将移动到滑动窗口中，并变成字典的一部分。
{{< /admonition >}}
</br>
{{< admonition note "短语字典">}}
 :black_nib:从字符序列S1...Sn，组成n个短语。比如字符(A,B,D) ,可以组合的短语为{(A),(A,B),(A,B,D),(B),(B,D),(D)},如果这些字符在滑动窗口里面，就可以记为当前的短语字典，因为滑动窗口不断的向前滑动，所以短语字典也是不断的变化。
{{< /admonition >}}
...
> :speech_balloon: 假设滑动窗口大小为10，向前缓冲区大小为6 
</br>
...
>:speech_balloon: 如上图，应输出02a00e46e,编码完成
---
##### 解码
> :speech_balloon: 编码过程逆向
...
```
{{< admonition tip>}}
:triangular_flag_on_post:使用了KMP算法来匹配最长字符串
{{< /admonition >}}
...
{{< admonition tip>}}
:warning:在最长匹配字符串比较多时，由于代码使用过多容器，会使解码速度下降，需要改进
{{< /admonition >}}

---
{{< admonition note "参考文章">}}
 :balloon: [图解LZ77压缩算法](https://mp.weixin.qq.com/s/XGzuSF-L-qKriyRGcGs-KA)
{{< /admonition >}}
